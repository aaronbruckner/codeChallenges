...
describe('postorderTraversal', () => {
  ...
  it('should return an array', () =>{
    assert.isArray(postorderTraversal(new Node(10)));
  });

  it('should return an empty array if node is invalid', () =>{
    assert.deepEqual(postorderTraversal(null), []);
  });
  ...
  describe('scenarios', () => {
    ...
    it('single root node with no children', () => {
      ...
      assert.deepEqual(postorderTraversal(root), expectedOrder);
    });
    ...
    it('root node with many children and grandchildren', () => {
      ...
      let expectedOrder = [3, 5, 8, 7, 5, 14, 13, 20, 15, 10];
      assert.deepEqual(postorderTraversal(root), expectedOrder);
    });
