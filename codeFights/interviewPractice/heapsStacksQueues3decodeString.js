...
function decodeString(s) {
    ...
    for (let i = 0; i < s.length; i++) {
        let char = s.charAt(i);
        if (char === ']') {
            // End of encoded string, pop and decode.
            let stringToDecode = '';
            while (true) {
                let decodePart = stack.pop();
                if (decodePart === '[') {
                    stack.push(stringToDecode.repeat(extractKFromStack(stack)));
                    break;
                } else {
                    stringToDecode = decodePart + stringToDecode;
                }
            }
        } else {
            stack.push(char);
...
function extractKFromStack(stack) {
    let k = '';
    let isDigit = new RegExp(/\d/);
    while(stack.length && isDigit.test(stack[stack.length - 1])) {
        k = stack.pop() + k;
    }
    return parseInt(k);
}
