...
function nearestGreater(a) {
    ...
    function sweepForward() {
        ...
        for (let i = 0; i < a.length; i++) {
            let value = a[i];
            result.push(-1);
            while (stack.length && value > a[stack[stack.length - 1]]) {
                result[stack.pop()] = i;
            }
            stack.push(i);
        }
    ...
    function sweepBackwards() {
        let stack = [];
        for (let i = a.length - 1; i >= 0; i--) {
            let value = a[i];
            while (stack.length && value > a[stack[stack.length - 1]]) {
                let lesserI = stack.pop();
                let fwdAns = result[lesserI];
                if (fwdAns === -1 || Math.abs(i - lesserI) <= Math.abs(fwdAns - lesserI)) {
                    result[lesserI] = i;
                }
            }
            stack.push(i);
